...
export default function Home() {
  ...
  useEffect(() => {
    if (searchQuery) { setCategory('All'); setPlatform('All') }
  }, [searchQuery])
  ...
  return (
    <main>
        ...
        <>
          ...
          <section className={styles.hero} aria-labelledby="hero-heading">
            <div className={styles.heroInner}>
              <div className={styles.heroContent}>
                ...
                <h1 id="hero-heading" className={styles.heroTitle}>
                  Get the scoop without the scroll!<br />
                </h1>
                <p className={styles.heroSubtitle}>
                  TrendWise keeps educators and parents informed about the social media trends, 
                  slang, and apps circulating among students — so you can have 
                  proactive conversations before issues arise.
                </p>
            ...
            <div className={styles.heroBorderBar} aria-hidden="true">
              {[ 'TikTok', 'Instagram', 'Discord', 'BeReal', 'YouTube'].map(t => (
                <span key={t} className={styles.heroBorderBarItem}>{t}</span>
              ))}
            </div>
          </section>
          ...
          <section className={styles.howStrip} aria-label="How TrendWise works">
            <div className={styles.howInner}>
              {[
                { icon: '📡', title: 'Trends monitored weekly', desc: 'We track whats circulating across TikTok, Instagram, Discord, and more.' },
                { icon: '✍️', title: 'Written for educators', desc: 'Plain language, no jargon. Reviewed for school-appropriate accuracy.' },
                { icon: '📬', title: 'Delivered every Monday', desc: 'A concise digest lands in your inbox, ready to share with colleagues or parents.' },
              ].map(item => (
          ...
          <section className={styles.section} aria-labelledby="trending-heading">
            <div className={styles.sectionHeader}>
              <div>
                ...
                <h2 id="trending-heading" className={styles.sectionTitle}>Trending Now</h2>
              </div>
      ...
      <div id="resources" className={styles.pageBody}>
        <section className={styles.filtersSection} aria-label="Filter resources">
          <div className={styles.filtersHeader}>
            <h2 className={styles.filtersTitle}>
              {searchQuery
                ? `Search results for "${searchQuery}"`
                : isFiltered ? 'Filtered Resources' : 'All Resources'}
            </h2>
            <div className={styles.filtersGroup}>
              <FilterBar label="Category" options={CATEGORIES} active={category} onChange={setCategory} />
              <FilterBar label="Platform" options={PLATFORMS} active={platform} onChange={setPlatform} />
            </div>
          </div>
        </section>
